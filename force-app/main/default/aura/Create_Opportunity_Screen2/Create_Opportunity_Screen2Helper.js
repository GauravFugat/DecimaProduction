({
    CreateOpportunityAndPR : function(component,event,helper,PricingRequest){
       // alert('Lookups ==>> '+JSON.stringify(component.get("v.selectedLookUpRecords")));
        component.set("v.spinner",true);
        console.log('alert 4==>>'+JSON.stringify(PricingRequest));
        ...
        var selectedLookupCons = component.get("v.selectedLookUpRecords");
        var ContactId = [];
        for(var i=0 ; i<selectedLookupCons.length ; i++){
            ContactId.push(selectedLookupCons[i].Id);
        }
       // alert('AccId-->> '+AccId);
       // alert('ContactId-->> '+JSON.stringify(ContactId));
        ...
        if(AccId != null && ContactId != null){
            if(PricingRequest.DueDate != ''){
                ...
                var selectedIndexContractId = component.get("v.selectedIndexContract") ? component.get("v.selectedIndexContract").Id : null;
                var selectedIndexOpportunityId = component.get("v.selectedIndexOpportunity") ? component.get("v.selectedIndexOpportunity").Id : null
                ...
                action.setParams({  
                    ...
                    'selectedLoadFollowing' : component.get("v.selectedLoadFollowing"),
                    'selectedIndexContractId' : selectedIndexContractId,
                    'selectedIndexOpportunityId' : selectedIndexOpportunityId
                });
